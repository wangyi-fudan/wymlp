...
static inline void CNAME(unsigned m, unsigned n, unsigned k, float alpha, float * __restrict__ A, float * __restrict__ B, float * __restrict__ C, unsigned LDC){
...
static inline void sgemm_tcopy_16(float *src, float *dst, unsigned lead_dim, unsigned dim_first, unsigned dim_second){
    if(dim_first==0 || dim_second==0) return;
    unsigned count_first,count_second;
    float *tosrc,*todst;
...
static inline void sgemm_ncopy_16(float *src, float *dst, unsigned lead_dim, unsigned dim_first, unsigned dim_second){
    if(dim_first==0 || dim_second==0) return;
    unsigned count_first,count_second,tosrc_inc;
...
static inline void sgemm_tcopy_4(float *src, float *dst, unsigned lead_dim, unsigned dim_first, unsigned dim_second){
    if(dim_first==0 || dim_second==0) return;
    unsigned count_first,count_second;
    float *tosrc,*todst;
    for(count_second=0;count_second<dim_second;count_second++){
...
static inline void sgemm_ncopy_4(float *src, float *dst, unsigned lead_dim, unsigned dim_first, unsigned dim_second){
    if(dim_first==0 || dim_second==0) return;
    unsigned count_first,count_second,tosrc_inc;
...
void inline sgemm(float alpha,float *a,float *b,float *c){
    ...
    for(k_count=0;k_count<k;k_count+=BLOCKDIM){
      ...
      if(!transb) { sgemm_ncopy_4(b_current_pos,b_buffer,ldb,k_subdim,n); b_current_pos += BLOCKDIM; }
      else { sgemm_tcopy_4(b_current_pos,b_buffer,ldb,n,k_subdim); b_current_pos += (int64_t)ldb * BLOCKDIM; }
      ...
      for(m_count=0;m_count<m;m_count+=BLOCKDIM){
        ...
        if(!transa) { sgemm_tcopy_16(a_current_pos,a_buffer,lda,m_subdim,k_subdim); a_current_pos += BLOCKDIM; }
        else { sgemm_ncopy_16(a_current_pos,a_buffer,lda,k_subdim,m_subdim); a_current_pos += (int64_t)lda * BLOCKDIM; }
